...
use crate::{link_ops::LinkOps, pointer_ops::PointerOps};
...
/// This allows links and containers to be separated and avoids the need for objects to be modified
/// to contain a link.
///
/// # Safety
///
...
pub unsafe trait Adapter {
    ...
    unsafe fn get_mut_value(
        &self,
        link: <Self::LinkOps as LinkOps>::LinkPtr,
    ) -> *mut <Self::PointerOps as PointerOps>::Value;
...
#[macro_export]
macro_rules! container_mut_of {
    ($ptr:expr, $container:path, $field:ident) => {
        #[allow(clippy::cast_ptr_alignment)]
        {
            ($ptr as *mut _ as *mut u8).sub($crate::offset_of!($container, $field))
                as *mut $container
        }
    };
}
...
macro_rules! intrusive_adapter {
    (@impl
        ...
        unsafe impl<$($args),*> $crate::Adapter for $name<$($args),*> $($where_)* {
            ...
            #[inline]
            unsafe fn get_mut_value(&self, link: <Self::LinkOps as $crate::LinkOps>::LinkPtr) -> *mut <Self::PointerOps as $crate::PointerOps>::Value {
                $crate::container_mut_of!(link.as_ptr() as *mut _, $value, $field)
            }
            #[inline]
