    ...
    A::LinkOps: RBTreeOps,
{
    ...
    #[inline]
    pub fn into_mut(self) -> Option<&'a mut <A::PointerOps as PointerOps>::Value> {
        Some(unsafe { &mut *self.tree.adapter.get_mut_value(self.current?) })
    }
}
...
// Allow sending to another thread if the ownership (represented by the <A::PointerOps as
// PointerOps>::Pointer owned pointer type) can be transferred to another thread.
...
mod tests {
    ...
    use std::{fmt, format, rc::Rc, vec, vec::Vec};
